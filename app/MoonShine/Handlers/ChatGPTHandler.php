...
class ChatGPTHandler extends Handler
{
    ...
    public static function process(): void
    {
        ...
        $reviews = Review::where('status', 'новый4')->get();
...
Представь, что ты лучший специалист по работе с отзывами на маркетплейсах. Проанализируй следующий отзыв, состоящий из достоинств, недостатков и комментариев. Имя клиента: "{$clientName}". Если имя указано, обязательно используй его в приветствии.

Определи:
- одну тематику, которая наилучшим образом характеризует отзыв;
- тональность отзыва (положительная, отрицательная, нейтральная);
- и составь персонализированный, человечный ответ, обязательно начиная с "{$greeting}".

🔽 Что должно включать ответ:
